...
(:action instantiate-recipe1role1resource
    :parameters (?recipe - recipe1role1resource ?flow - recipeFlow ?action - action ?role1 - actor ?resource1 - resource ?location - location)
    :precondition (and 
...
(:action instantiate-recipe2roles1resource
    :parameters (?recipe - recipe2roles1resource ?flow - recipeFlow ?action - action ?role1 ?role2 - actor ?resource1 - resource ?location - location)
    :precondition (and 
        (not (= ?role1 ?role2))
        (recipeClauseOf ?flow ?recipe)
        (flowResource ?flow resource1)
...
(:action instantiate-recipe2roles2resources
    :parameters (?recipe - recipe2roles2resources ?flow - recipeFlow ?action - action ?role1 ?role2 - actor ?resource1 ?resource2 - resource ?location - location)
    :precondition (and 
        (not (= ?role1 ?role2))
        (not (= ?resource1 ?resource2))
        (recipeClauseOf ?flow ?recipe)
        (or
